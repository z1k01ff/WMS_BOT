...
def privyazka_DB(product_nr):
    ...
    dfi.export(df, 'PNG/privyazka.png')
    print(product_nr)
    print(df)
    return df
...
def soderzimoe_DB(product_nr):
    ...
    df = pd.read_sql(sql, connection)
    dfi.export(df, 'PNG/soderzimoe.png')
    print(product_nr)
    print(df)
    return df
...
def vidpravka_DB():
    ...
    dfi.export(piec_df, 'PNG/vidpravka_piec.png')
    dfi.export(pack_df, 'PNG/vidpravka_pack.png')
...
def transport_vidpravka_marshrut():
    ...
    df = pd.read_sql(sql, connection)
    dfi.export(df, 'PNG/transport_vidpravka.png')
    print(df)
...
def vidpravka_nabrano(date_started):
    ...
    df = df._append(sum_df, ignore_index=True)
    dfi.export(df, 'PNG/vidpravka_nabrano.png')
    print(df)
...
def transport_peremish_in():
    sql = f"select sp_to_nr as MISCE, COUNT(sp_to_nr) as KILKIST from TRV_SPREAD_TRANS_ORDERS_FAST " \
          ...
          f"GROUP by sp_to_nr"
    df = pd.read_sql(sql, connection)
    dfi.export(df, 'PNG/transport_perem_IN.png')
    print(df)
